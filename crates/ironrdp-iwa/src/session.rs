...
use crate::direct_socket::DirectSocket;
...
type RdpStream = futures_rustls::client::TlsStream<DirectSocket>;
...
impl iron_remote_desktop::SessionBuilder for SessionBuilder {
    ...
    async fn connect(&self) -> Result<Self::Session, Self::Error> {
        ...
        let (socket, direct_host) = DirectSocket::connect(&destination)
            .await
            .context("couldn't open an IWA Direct Socket")
...
/// Windows RDP commonly presents a self-signed certificate. The TLS channel is
/// still encrypted and its certificate public key is bound into CredSSP below,
/// but this verifier deliberately does not establish PKI trust. The IWA UI
/// makes that limitation explicit before credentials are sent.
#[derive(Debug)]
struct AcceptAnyRdpCertificate {
    supported: futures_rustls::rustls::crypto::WebPkiSupportedAlgorithms,
}

impl AcceptAnyRdpCertificate {
    fn new(provider: Arc<futures_rustls::rustls::crypto::CryptoProvider>) -> Self {
        Self {
...
impl futures_rustls::rustls::client::danger::ServerCertVerifier for AcceptAnyRdpCertificate {
    fn verify_server_cert(
        &self,
        _end_entity: &futures_rustls::pki_types::CertificateDer<'_>,
        _intermediates: &[futures_rustls::pki_types::CertificateDer<'_>],
    ...
    fn verify_tls12_signature(
        &self,
        message: &[u8],
        cert: &futures_rustls::pki_types::CertificateDer<'_>,
        ...
        futures_rustls::rustls::crypto::verify_tls12_signature(message, cert, dss, &self.supported)
    }
    ...
    fn verify_tls13_signature(
        ...
        futures_rustls::rustls::crypto::verify_tls13_signature(message, cert, dss, &self.supported)
    }
...
async fn connect(
    ...
    let mut framed = ironrdp_futures::LocalFuturesFramed::new(socket);
    ...
    let socket = framed.into_inner_no_leftover();

    let provider = Arc::new(oxitls_rustcrypto_provider::provider());
    let tls_config = futures_rustls::rustls::ClientConfig::builder_with_provider(provider.clone())
        .with_safe_default_protocol_versions()
        .context("configure TLS protocol versions")?
        .dangerous()
        .with_custom_certificate_verifier(Arc::new(AcceptAnyRdpCertificate::new(provider)))
        .with_no_client_auth();
    let tls_server_name =
        futures_rustls::pki_types::ServerName::try_from(direct_host.clone()).context("invalid TLS server name")?;
    let tls_stream = futures_rustls::TlsConnector::from(Arc::new(tls_config))
        .connect(tls_server_name, socket)
        .await
        .context("TLS handshake with RDP host failed")?;
    ...
    let server_cert = tls_stream
        ...
        .context("RDP server did not provide a TLS certificate")?;
    verify_or_trust_certificate(&destination, server_cert.as_ref())?;
    let cert =
        x509_cert::Certificate::from_der(server_cert.as_ref()).context("failed to decode RDP server certificate")?;
    let server_public_key = cert
        .tbs_certificate()
        .subject_public_key_info()
        .subject_public_key
        .as_bytes()
        .context("RDP server public key BIT STRING is not aligned")?
        .to_vec();

    let upgraded = ironrdp_futures::mark_as_upgraded(should_upgrade, &mut connector);
    let mut framed = ironrdp_futures::LocalFuturesFramed::new(tls_stream);
...
fn verify_or_trust_certificate(endpoint: &str, certificate_der: &[u8]) -> anyhow::Result<()> {
    ...
    let fingerprint = digest
        ...
        .collect::<Vec<_>>()
        .join(":");
    let storage_key = format!("rdp.certificate.{}", endpoint.to_ascii_lowercase());
        ...
        .and_then(|storage| storage.get_item(&storage_key).ok().flatten())
    {
        ...
        anyhow::bail!(
            "RDP server certificate changed for {endpoint}. Expected SHA-256 {expected}, received {fingerprint}. Refusing to send credentials"
        );
    }

    let accepted = window
        .confirm_with_message(&format!(
            "首次连接到 {endpoint}。请核对 RDP 证书 SHA-256 指纹：\n\n{fingerprint}\n\n确认信任并继续发送登录凭据吗？"
        ))
        .unwrap_or(false);
    if !accepted {
        anyhow::bail!("RDP server certificate was not trusted by the user");
    }

    if let Some(storage) = storage {
        let _ = storage.set_item(&storage_key, &fingerprint);
    }
    Ok(())
}
